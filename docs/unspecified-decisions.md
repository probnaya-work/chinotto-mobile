...
| 8.13 | **Retained audio cannot be played** | The chip is drawn, it is tappable, and it toggles between a play mark and a stop mark — and nothing is heard, because there is no player. No audio package is installed and `onPlay` only sets `playingId`. The recordings are on disk and `audioMissing` correctly reports when they are not, so the material is intact; the surface just offers something it cannot do. Found on the simulator: a chip showed a stop mark for over a minute for twenty-five seconds of audio, because nothing ever finished. Wiring a player is an addition, not a repair, so it is named here rather than bundled into a fixing pass. |
