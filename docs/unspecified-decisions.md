...
| 8.15 | **Haptics** | The shipping app used `expo-haptics` on several v1 surfaces and had a preference for it in `storage/settingsPrefs.ts`. Those surfaces are gone, and the redesign's prototype specifies no haptic feedback anywhere — so there is nothing to reconnect it *to* without deciding, newly, which moments in the record should be felt. The package is still installed and nothing has been ripped out. Named here rather than invented: adding feedback patterns the design does not describe would be designing, and dropping it silently would be losing something the old app had. |
