...
| 8.14 | **Nothing sweeps orphaned audio** | Retained audio is deleted when a recording is too short to keep and when a removal is finally published; there is no pass for a file whose fragment never came to exist. That should not happen any more — it was a symptom of the voice controller being rebuilt mid-recording — but the two files it left on the test simulator are the proof that it can, and there is nothing that would find them. A sweep needs a rule about what "orphaned" means that is safe against a half-written capture, which is a decision, not an oversight to fix in passing. |
