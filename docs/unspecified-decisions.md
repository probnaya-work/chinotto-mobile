...
| 8.17 | **Share and widget both need a real signature — traced to the exact link that fails** | The extension is built into the bundle, appears in the system share sheet, is invoked by sharing from Safari, receives the URL (`SLComposeServiceViewController got NSURL … error: (null)`), serialises the right payload — `[{"type":"url","mimeType":"text/html","value":"https://www.theatlantic.com/…"}]` — and writes it to `UserDefaults(suiteName: "group.com.chinotto.mobile")`. The app then reads that suite and finds nothing. **Why:** on a simulator build the app group is not backed by a provisioning profile, so `cfprefsd` gives each process its own store. Both copies exist on disk, in the extension's sandbox and in the app's, and never meet. Re-signing the simulator build ad-hoc with the entitlement is not enough: it makes `containermanagerd` create the group directory (verified — `container_create_or_lookup…: success`) but does not make the preferences suite shared. **The widget has the identical dependency:** `chinotto_widget_recent_thoughts_v1` is likewise written into the app's own sandbox, so the widget cannot read it on a simulator either. Everything either side of the suite is verified: the payload shape the extension produces is now a test (`share.test.ts`), the field mapping to JS matches (`type` → `shareType`), and the intake is covered. Both resolve on the signed device build, whose entitlements are applied — verified in the signature of the app and of both extensions. |
